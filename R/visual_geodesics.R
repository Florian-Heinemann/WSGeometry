...
#' plotGeodesic(Geo,File="TestGeodesicDefault")
#' plotGeodesic(Geo,method="bin",File="TestGeodesicDefaultBin")
...
#' M<-40
#' U<-runif(1,0.5,1)
#' Torus<-WSGeometry:::gen_torus(M,U,min(U/2,runif(1)))
#' v<-WSGeometry:::normalize(runif(3))
#' Torus<-WSGeometry:::rotate3D(Torus,v,runif(1,0,2*pi))
...
#' plotGeodesic(geo,File="3dGeodesic")
#' }
#' @export
plotGeodesic<-function(Geodesic,method="default",images=TRUE,gif=TRUE,File="Geodesic",resolution=c(400,400),dotsize=2,gridsize=c(100,100),out.col= grey(0:1000/1000),splitrange=c(1,1),turn=FALSE,phi=40,theta=40,fps=NULL){
  L<-length(Geodesic)
