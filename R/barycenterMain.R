...
#' WSGeometry:::frechet_func(res1,data.list)
...
#' WSGeometry:::frechet_func(res2,data.list)
...
#' WSGeometry:::frechet_func(res3,data.list)
...
#' WSGeometry:::frechet_func(res4,data.list)
#' ##Visual Examples
#' ###ellipses
#' set.seed(420)
#' N<-20
...
#'   I<-WSGeometry:::bin2d(data.list[[i]]$coordinates,data.list[[i]]$mass,c(L*2,L*2))
#'   I<-WSGeometry:::smear(I,1,1)
#'   I<-I/sum(I)
#'   image.list[[i]]<-I
#' }
#' 
...
#' output.supp = WSGeometry:::grid_positions(2*L,2*L)))
#' plot(res2)
