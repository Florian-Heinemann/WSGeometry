umaaipm_free_wrap<-function(pos.list,weights.list,support,C,maxIter=100,outermaxIter=10,startvec=NULL,thresh=10^-5,threads=1){
  t<-Matrix::t
  d<-dim(pos.list[[1]])[2]
  N<-length(pos.list)
  ...
  gc() 
  x<-umaaipm_free_cpp(p,s,x,b,costvec,support,Reduce("cbind",pos.list), const,N,m,M,sizes,sizes_csum,n_row, n_col,build_U(N,m),C,maxIter,outermaxIter,thresh,threads)
  L<-length(x[[2]])
  return(list(positions=(x[[1]][,1:(m-1)]),weights=mass_cap*x[[2]][(L-m+1):(L-1)]))
}